...
def gen_test_input_data():
    def gen_tensor_shape_type_place(shape, dtype, place='npu'):
        ...
        x = paddle.randn(shape=shape, dtype=dtype)
        return x
    ...
    def gen_scalar_tensor(value, dtype, place):
        paddle.set_device(place)
...
def test():
    RESULT_FILE_NAME = "accuracy_checking_result_" + "test" + ".csv"
    DETAILS_FILE_NAME = "accuracy_checking_details_" + "test" + ".csv"
    compare  =Comparator(RESULT_FILE_NAME,DETAILS_FILE_NAME,False)
    inputs = gen_test_input_data()
    for i, ele in enumerate(inputs):
        full_api_name, bench_output, device_output = ele
        is_fwd_success, is_bwd_success = compare.compare_output(full_api_name, bench_output,device_output)
        print(i,is_fwd_success,is_bwd_success)
        # _, api_name, _ = full_api_name.split('*')
        # status, compare_result, message = compare._compare_core(api_name, bench_output, device_output)
        # print(i,api_name,status,compare_result)
